  ...
  export class PtrArray {
    ...
    get entries(): Array<GenesysObject.Atype> {
      ...
      for (let i = 0; i < (this.amt as any); i++) {
        this._entries.push(new GenesysObject.Atype(this._io, this, this._root, (this.dtype as any)));
      }
      this._io.seek(_pos);
      return this._entries;
    }
...
export namespace GenesysObject {
  export class CgsResourceThandle {
    ...
    _read() {
      this.handle = (new GenesysObject.CgsResourceHandle(this._io, this, this._root)) as any
    }
